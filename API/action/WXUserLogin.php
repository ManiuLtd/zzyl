...
if (isset($_REQUEST['userID']) && !empty($_REQUEST['userID']) && isset($_REQUEST['unionID']) && !empty($_REQUEST['unionID'])) {
    ...
    //代理邀请玩家 绑定的玩家赠送的奖励
    $bindAgent_sendUser_money    = (int) $c['bindAgent_sendUser_money'];
    $bindAgent_sendUser_diamonds = (int) $c['bindAgent_sendUser_diamonds'];

    //代理邀请玩家 代理的奖励 无奖励

    //玩家邀请玩家 绑定的玩家赠送的奖励
    $user_invitationUser_SendMoney    = (int) $c['user_invitationUser_SendMoney'];
    $user_invitationUser_SendDiamonds = (int) $c['user_invitationUser_SendDiamonds'];

    //玩家邀请玩家 邀请的玩家赠送的的奖励
    $userBind_sendMoney    = (int) $c['userBind_sendMoney'];
    $userBind_sendDiamonds = (int) $c['userBind_sendDiamonds'];
    ...
    if ($sharCode && $sharCode['status'] == 0) {
        ...
        if ($sharCode['agentid'] == 0) {
            // 没有邀请码 玩家邀请玩家
            // 发送奖励
            $packet1 = $send->makeSendAgent($userID, 1, $userBind_sendMoney); // 被邀请的玩家 送金币
            $packet1 = $send->makeSendAgent($userID, 2, $userBind_sendDiamonds); // 被邀请的玩家 送钻石

            $packet3 = $send->makeSendAgent($sharCode['agent_userid'], 1, $user_invitationUser_SendMoney); //邀请的玩家 送金币
            $packet4 = $send->makeSendAgent($sharCode['agent_userid'], 2, $user_invitationUser_SendDiamonds); //邀请的玩家 送钻石

            if ($userBind_sendMoney > 0) {
                $res1 = $socket->send($send::AgentID, 1, 0, $packet1);
                if (!$res1) {
                    $arr = array('status' => 0, 'msg' => '请求失败，1与服务器通信出现错误');
                    exit(return_json($arr));
                }
            }

            if ($userBind_sendDiamonds > 0) {
                $res2 = $socket->send($send::AgentID, 1, 0, $packet2);
                if (!$res2) {
                    $arr = array('status' => 0, 'msg' => '请求失败，2与服务器通信出现错误');
                    exit(return_json($arr));
                }
            }

            if ($user_invitationUser_SendMoney > 0) {
                $res3 = $socket->send($send::AgentID, 1, 0, $packet3);
                if (!$res3) {
                    $arr = array('status' => 0, 'msg' => '请求失败，3与服务器通信出现错误');
                    exit(return_json($arr));
                }

            }

            if ($user_invitationUser_SendDiamonds > 0) {
                $res4 = $socket->send($send::AgentID, 1, 0, $packet4);
                if (!$res4) {
                    $arr = array('status' => 0, 'msg' => '请求失败，4与服务器通信出现错误');
                    exit(return_json($arr));
                }
            }
            ...
            //$db1 = db::getInstance();
            $sql = "update web_bindagentid set status=1,userid={$userID} where id={$sharCode['id']}";
            $res = $db->getRow($sql);
            // 进行统计
            $sql1 = "insert into web_codeInvitation values({$userID}," . time() . ",{$user_invitationUser_SendMoney},{$sharCode['agent_userid']},0,$user_invitationUser_SendDiamonds)";
            $sql2 = "insert into web_codeInvitation values({$sharCode['agent_userid']}," . time() . ",{$userBind_sendMoney},0,0,$userBind_sendDiamonds)";
            $db->getRow($sql1);
            $db->getRow($sql2);
	...
	if (!$res) {
        ...
        $packet1 = $send->makeSendAgent($userID, 1, $bindAgent_sendUser_money); // 赠送给玩家金币
        $packet2 = $send->makeSendAgent($userID, 2, $bindAgent_sendUser_diamonds); // 赠送给玩家钻石
        //$packet2 = $send->makeRechargePacket($userID, $bindAgent_sendUser_diamonds, 2);

        $res1 = $socket->send($send::AgentID, 1, 0, $packet1);
        $res2    = $socket->send($send::AgentID, 1, 0, $packet2);
        //$res2 = $socket->send($send::RechargeID, 1, 0, $packet2);
        ...
        $sql1 = "insert into web_codeInvitation values({$userID}," . time() . ",{$bindAgent_sendUser_money},{$sharCode['agent_userid']},1,$bindAgent_sendUser_diamonds)";
