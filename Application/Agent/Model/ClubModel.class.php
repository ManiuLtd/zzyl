...
class ClubModel extends Model
{
    ...
    public function clubList($map = [])
    {
        ...
        $list  = M()
            ...
            ->where($map)
            ->field('g.*,u.name as username,(select count(*) from web_club_member as m where g.friendsGroupID=m.friendsGroupID ) as peoplecount')
    ...
    {
        $User  = M()->table('web_club as f');
        $count = $User
            ->join('left join userInfo as u on u.userID=f.masterID')
            ->where($map)
            ->field('f.*,u.name  as username')
            ->count();
        $Page = new \Think\Page($count, 25);
        $show = $Page->show();
        $list = M()->table('web_club_member as f')
        ...
        foreach ($list as &$member) {
            ...
            $member = ClubModel::getInstance()->getFriendsGroupMember($member['friendsgroupid'], $member['userid']);
