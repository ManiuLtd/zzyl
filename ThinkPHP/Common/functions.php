...
function general_token() {
    //    在有效时间内，缓存数据，否则，生成新的token
    $token = cookie('x_token');
    $expire = 7200;
    if (!$token) {
        $time = time();//cookie('x_time');
//        $time = time();
        $token = getRandomStr(12);
        cookie('x_token', $token, ['expire' => $expire]);
//        cookie('x_time', $time, ['expire' => $expire - 10]);
    }
...
function verifyToken($strToken, $time = 0) {
    $token = getToken();
    if ($token['token'] == $strToken) {
        return true;
    }
    return false;
}
...
function getRandomStr($len = 6) {
    $str = 'abcdefghijklmnopqrstuvwsyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890';
    $str = str_shuffle($str);
