...
<head>
	...
	<style type="text/css">
		...
		.menpais img, .cardlist img { width: 40px; cursor: pointer; margin: 2px; }
...
<div class="container">
	<div class="row well page-header">
		<div class="col-xs-2">
			<select class="form-control players-count" id="playernum">
				<option value="2">2 人麻将</option>
				<option value="3">3 人麻将</option>
				<option value="4" selected="selected">4 人麻将</option>
			</select>
		</div>
	...
	<div class="alert alert-info">当前发牌对象：门牌<strong class="label label-danger current-menpai">1</strong></div>
	<div class="row jumbotron menpais">
		<div class="media">
			<h4 class="media-middle pull-left"><a class="btn btn-info" data-menpaiid=1>门牌头</a></h4>
			<div class="media-body menpai-cards"></div>
		</div>
		<div class="media">
			<h4 class="media-middle pull-left"><a class="btn btn-info" data-menpaiid=2>门牌尾</a></h4>
			<div class="media-body menpai-cards"></div>
		</div>
	</div>
	...
	$.fn.extend({
		...
		cardsInit: function(options) {
			...
			$.each(opt.cardsConf, function(i, n){
				...
				_this.append(objImg);
				if ($.inArray(n, [9,19,29,37]) != -1) { //换行判断
					_this.append($('<br/>'));
				}
			...
			$('a[data-menpaiid]').on('click', function(){
				...
				if(1 == menpaiid)
				{
					$('.current-menpai').text("头");
				}
				else if(2 == menpaiid)
				{
					$('.current-menpai').text("尾");
				}
